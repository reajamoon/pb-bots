...
export default {
  ...
  async execute(reaction, user) {
    try {
      ...
      if (!guild) return;
      const member = await guild.members.fetch(user.id).catch(() => { return null; });
      if (!member) return;
      const role = guild.roles.cache.get(targetRoleId) || await guild.roles.fetch(targetRoleId).catch(() => { return null; });
      if (!role) return;
