...
Sprint start grace period (configurable):
- When a sprint is created, it may optionally start after a configurable delay (default: 1 minute).
- Dean posts a short “Sprint starting in {DELAY_MINUTES} minutes” message and applies the grace buffer before the timer begins.
- During the buffer, members can do setup without losing sprint time (example: set baseline, link a project, review the sprint label).
- After the buffer ends, the sprint timer begins and the sprint is considered “in progress” for all time calculations.

Optional pre-start reminder pings (team coordination):
- Default: no pings during the grace buffer.
- If the host explicitly enables pre-start pings for this sprint, Dean may ping sprint participants during the grace buffer as reminders.
- Reminder schedule:
  - If {DELAY_MINUTES} >= 10: ping at 10 minutes remaining.
  - If {DELAY_MINUTES} >= 5: ping at 5 minutes remaining.
  - Always ping at 1 minute remaining (if pings are enabled and the delay is at least 1 minute).
- If the delay is longer than 10 minutes, additional pings may be sent every 5 minutes, but never more frequently than every 5 minutes, and never after the 1-minute warning.
...
Sprint start grace period (configurable):
- When a team sprint is hosted, it may optionally start after a configurable delay (default: 1 minute).
- Dean posts “Sprint starting in {DELAY_MINUTES} minutes” and applies the grace buffer before the host timer begins.
- During that buffer:
  - joiners can join without losing sprint time,
  - baseline prompts should happen inside that buffer.
- At buffer end, the sprint begins; join time tracking starts when each joiner joins, but the host sprint timer is based on the post-buffer start.
...
- When starting a sprint, the starter may optionally provide a **starting wordcount baseline** `B` (an absolute number from their document/app). If omitted, the baseline defaults to **0**.
...
- The baseline is the reference point used to interpret absolute totals and to display absolute start/end. The sprint’s displayed progress is always relative to that reference point.
...
Important clarification (prevents double-counting):
- `B` is the absolute total you are treating as your sprint's "starting line".
- If you end a sprint and immediately start another one, the new sprint must use your most recent absolute total as its new baseline, so the next sprint starts at 0 again.
- Example: baseline `B=30,000`, you report `A=31,000` so sprint progress is `1,000`. If you quick-restart and then report `A=32,000`, the new sprint baseline should default to `31,000`, so progress is `1,000` (not `2,000`).
...
- If the user quick-restarts immediately after a sprint, the new sprint should default its baseline to the participant's most recent known absolute total from the sprint they just finished.
  - If the participant never reported an absolute total for the prior sprint, fall back to carrying over the prior sprint's baseline plus the prior sprint's final sprint-total (0-based progress).
- This must be explained clearly in the restart UI so members understand why absolute totals keep working across back-to-back sprints and do not double-count the prior sprint.
